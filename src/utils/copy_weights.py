...
def copy_dataset_encoder_weights_from_pretrained_agent(
    args, checkpoint_current_model, game
):
    if args.path_to_pretrained_dataset_encoder:
        checkpoint_path_dataset_encoder = (
            ROOT_DIR / args.path_to_pretrained_dataset_encoder
        )
        if checkpoint_path_dataset_encoder.suffix != "":
            raise RuntimeError(
                f"Your path to the complete model has an suffix: {checkpoint_path_dataset_encoder.suffix} \n "
        ...
        for selpath in selected_path:
            ...
            assign_to_path(
                chkpoint_obj=checkpoint_current_model,
                path=selpath.split("/"),
                value=value,
            )
...
def assign_to_path(chkpoint_obj, path: str, value):
    ...
    if next_path == ".ATTRIBUTES":
        ...
        assign_to_path(
            chkpoint_obj=chkpoint_obj[int(next_path)], path=path[1:], value=value
        )
    elif hasattr(chkpoint_obj, next_path):
        assign_to_path(
            chkpoint_obj=getattr(chkpoint_obj, next_path), path=path[1:], value=value
        )
    else:
        raise RuntimeError(
            f"assign_to_path::error:: handling of path of form {next_path} with full path {path} not available"
        )
