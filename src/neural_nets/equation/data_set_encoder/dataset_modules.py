...
class MHSA(tf.keras.Model):
    ...
    def __init__(self, dim_in, dim_emb, dim_out, kwargs):
        ...
        self.mab = MAB(
            dim_Q=dim_in, dim_KV=dim_in, dim_emb=dim_emb, dim_out=dim_out, kwargs=kwargs
        )
...
class MAB(tf.keras.Model):
    ...
    def __init__(self, dim_Q, dim_KV, dim_emb, dim_out, kwargs):
        """
        ...
        * `F_A` is dimension of the embedding at input
        ...
        self.fc_q = tf.keras.layers.Dense(
            units=dim_emb, activation=None, name=f"fc_q_dense", use_bias=False
        )
        self.fc_k = tf.keras.layers.Dense(
            units=dim_emb, activation=None, name=f"fc_k_dense", use_bias=False
        )
        self.fc_v = tf.keras.layers.Dense(
            units=dim_emb, activation=None, name=f"fc_v_dense", use_bias=False
        )
        self.fc_mix_heads = (
            tf.keras.layers.Dense(
                units=dim_out, activation="gelu", name="fc_mix_heads_dense"
            )
            if mix_heads
            else None
        )
        self.fc_res = (
            tf.keras.layers.Dense(dim_out, activation="gelu", name=f"res_dense")
            if sep_res_embed
            else None
        )
        ...
        self.hidden_dropout = tf.keras.layers.Dropout(
            rate=self.kwargs["model_hidden_dropout_prob"],
            name="Dropout_Hidden" if self.kwargs["model_hidden_dropout_prob"] else None,
        )

        self.att_scores_dropout = tf.keras.layers.Dropout(
            rate=self.kwargs["model_att_score_dropout_prob"],
            name=(
                "Dropout_Att_Scores"
                if self.kwargs["model_att_score_dropout_prob"]
                else None
            ),
        )
    ...
    def init_rff(self, dim_out, rff_depth):
        ...
        self.rff = [
            tf.keras.layers.Dense(
                units=4 * dim_out, activation="gelu", name=f"DenseRff_{rff_depth}"
            ),
        ...
        for i in range(rff_depth - 1):
            self.rff += [
                tf.keras.layers.Dense(
                    units=4 * dim_out, activation=None, name=f"DenseRff_{i}"
                ),
                tf.keras.activations.gelu,
            ]
    ...
    def call(self, X, Y, *args, **kwargs):
        ...
        if self.fc_res is None:
            ...
            X_res = self.fc_res(
                X, training=kwargs["training"]
            )  # Separate embedding for residual
        ...
        Q_ = tf.concat(
            tf.split(
                value=Q, num_or_size_splits=int(Q.shape[3] / self.dim_split), axis=3
            ),
            1,
        )
        K_ = tf.concat(
            tf.split(
                value=K, num_or_size_splits=int(K.shape[3] / self.dim_split), axis=3
            ),
            1,
        )
        V_ = tf.concat(
            tf.split(
                value=V, num_or_size_splits=int(V.shape[3] / self.dim_split), axis=3
            ),
            1,
        )
        ...
        multihead_split = tf.split(
            value=multihead,
            num_or_size_splits=int(multihead.shape[1] / Q.shape[1]),
            axis=1,
        )
        multihead_concat = tf.concat(values=multihead_split, axis=3)
        ...
        if not self.pre_layer_norm and self.ln1 is not None:
            expanded_linear_H = self.ln1(expanded_linear_H, training=kwargs["training"])
