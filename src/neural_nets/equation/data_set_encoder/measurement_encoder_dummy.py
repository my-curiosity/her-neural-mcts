...
def reshape_measurements(data, column_names, measurement_in_row):
    ...
    tensor_with_row = tf.reshape(
        tensor=stacked_tensor, shape=(batch_size, -1, measurement_in_row)
    )
    return tensor_with_row
...
def reshape_measurements_to_pandas(data, column_names, measurement_in_row):
    columns = [
        feature.split("_row_")[0] for feature in column_names if "row_0" in feature
    ]
