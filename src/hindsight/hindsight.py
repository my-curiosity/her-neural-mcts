...
class Hindsight():
    ...
    def create_hindsight_history(self, final_state):
        ...
        try:
            ...
            self.logger.debug(f"In the calculation of the hindsight a FloatingPointError occur"
                             f"the equation is: {final_state.syntax_tree.rearrange_equation_prefix_notation(new_start_node_id=-1)[1]} \n"
                             f"the dataset is: {final_state.observation['data_frame']} ")
            return[],[]
        except RuntimeError as e:
            self.logger.debug(f"In the calculation of the hindsight a RuntimeError occur."
                              f"The error is {e} "
                              f"No history is added")
            return[],[]
        except NonFiniteError:
            self.logger.debug(f"In the calculation of the hindsight a NonFinite Element occur."
                              F"No history added")
            return [],[]
    ...
    def add_forward_history(self, history_forward, previous_state, state):
        ...
        """
        if self.mcts:
            possible_actions =np.nonzero(self.mcts.valid_moves_for_s[previous_state.hash])[0]
            if state.reward <0.5:
            ...
            history_forward.capture(
                ...
                pi=mcts_distribution,
                r=state.reward,
                v=None
            )

    def backward_pass(self, hindsight_data, history_forward, history_backward, previous_state, state):
        """
        ...
        state_hindsight.observation['true_equation_hash'] = self.hindsight_true_equation_hash
        ...
        history_backward.capture(
            state=state_hindsight,
            pi=hindsight_distribution,
            r=1,
            v=None
        )
        return history_forward, history_backward, hindsight_data
    ...
    def forward_pass(self, actions, history_forward, history_backward, previous_state):
        if check_for_first_state(previous_state):
            hindsight_data = self.create_hindsight_data(
                actions=actions,
                original_data=previous_state.observation['data_frame']
            )
        else:
            # call previous state
            history_forward, history_backward, hindsight_data = self.visit_previous_state(
                state=previous_state,
                history_forward=history_forward,
                history_backward=history_backward,
                actions=actions
    ...
    def create_hindsight_data(self, actions, original_data):
        ...
        if np.all(np.isfinite(y_calc)):
            ...
            self.logger.info(f"In calculation of hindsight  y_calc a non finite element happen. ")
            raise RuntimeError
