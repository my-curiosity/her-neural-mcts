...
class Terminal(AbstractOperator):
    ...
    def residual(self, call_node_id, dataset, kwargs):
        res = self.node.parent_node.math_class.residual(
    ...
    def evaluate_subtree(self, call_node_id, dataset, kwargs):
        ...
        if symbol in dataset.columns:
            ...
            return np.full(shape=dataset.shape[0], fill_value=symbol, dtype=np.float64)

    def delete(self):
        pass

    def operator_data_range(self, variable):
        min_value, max_value, depends_on_variable = super(
            Terminal, self
        ).operator_data_range(variable)
