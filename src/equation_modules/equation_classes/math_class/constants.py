...
class Constants(AbstractOperator):
    def __init__(self, node):
        ...
        if (
            self.node.tree.num_constants_in_complete_tree
            > self.node.tree.args.max_constants_in_tree
        ):
    ...
    def evaluate_subtree(self, call_node_id, dataset, kwargs):
        ...
        return np.full(
            shape=dataset.shape[0], fill_value=kwargs[symbol]["value"], dtype=np.float64
        )
