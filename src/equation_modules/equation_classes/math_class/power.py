from src.equation_modules.equation_classes.math_class.abstract_operator import AbstractOperator
import numpy as np
...
class Power(AbstractOperator):
    ...
    def prefix_notation(self, call_node_id, kwargs):
        if call_node_id == self.node.node_id:
            ...
            c_0_str = self.node.list_children[0].math_class.prefix_notation(
                self.node.node_id, kwargs
            )
            c_1_str = self.node.list_children[1].math_class.prefix_notation(
                self.node.node_id, kwargs
            )
    ...
    def infix_notation(self, call_node_id, kwargs):
        if call_node_id == self.node.node_id:
            p_str = self.node.parent_node.math_class.infix_notation(
                call_node_id=self.node.node_id, kwargs=kwargs
            )
            return p_str
        elif call_node_id == self.node.parent_node.node_id or call_node_id is None:
            c_0_str = self.node.list_children[0].math_class.infix_notation(
                self.node.node_id, kwargs
            )
            c_1_str = self.node.list_children[1].math_class.infix_notation(
                self.node.node_id, kwargs
            )
            return f"  {c_1_str}  **" f"  {c_0_str}  "
        elif call_node_id == self.node.list_children[0].node_id:
            p_str = self.node.parent_node.math_class.infix_notation(
                call_node_id=self.node.node_id, kwargs=kwargs
            )
            ...
            return f" log ( {p_str} ) / " f" log ( {c_1_str} ) "
        elif call_node_id == self.node.list_children[1].node_id:
            p_str = self.node.parent_node.math_class.infix_notation(
                call_node_id=self.node.node_id, kwargs=kwargs
            )
    ...
    def residual(self, call_node_id, dataset, kwargs):
        if call_node_id == self.node.list_children[0].node_id:
            ...
            return np.divide(
                np.log(p, dtype=np.float64),
                np.log(c_1, dtype=np.float64),
                dtype=np.float64,
            )
        elif call_node_id == self.node.list_children[1].node_id:
            p = self.node.parent_node.math_class.residual(
            ...
            return np.power(p, np.divide(1, c_0, dtype=np.float64), dtype=np.float64)
    ...
    def evaluate_subtree(self, call_node_id, dataset, kwargs):
        child_0 = self.node.list_children[0].math_class.evaluate_subtree(
            self.node.node_id, dataset, kwargs
        )
        child_1 = self.node.list_children[1].math_class.evaluate_subtree(
            self.node.node_id, dataset, kwargs
        )
